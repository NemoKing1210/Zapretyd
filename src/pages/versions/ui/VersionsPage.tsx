...
export function VersionsPage({
  ...
  return (
    <Stack spacing={3}>
      ...
      {latest && (
        <Card sx={{ borderColor: latest.isNewerThanInstalled ? 'primary.main' : undefined }}>
          <CardContent>
            <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" gap={2}>
              <Box>
                <Typography variant="overline">{t('versions.latestRelease')}</Typography>
                <Typography variant="h5">{latest.tag}</Typography>
              ...
              {latest.isNewerThanInstalled ? (
                <Button
                  startIcon={busy ? <CircularProgress size={18} /> : <DownloadOutlined />}
                  disabled={busy}
                  onClick={onInstall}
                >
                ...
                <Chip label={t('versions.alreadyInstalled')} color="success" />
              )}
            </Stack>
          </CardContent>
        </Card>
      ...
      <Stack spacing={2}>
        {versions.length === 0 && <Alert severity="info">{t('versions.noVersions')}</Alert>}
